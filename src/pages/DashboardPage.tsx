...
const StatCard = ({ label, value, color, delta, deltaUp }: { label: string; value: string | number; color: string; delta: string; deltaUp: boolean }) => (
  <div style={{ background: 'var(--navy3)', border: '1px solid var(--border)', borderRadius: 12, padding: 16, position: 'relative', overflow: 'hidden' }}>
    ...
    <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(20px, 4vw, 28px)', fontWeight: 700, color, lineHeight: 1 }}>{value}</div>
...
export default function DashboardPage() {
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: 'clamp(12px, 3vw, 20px)' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(18px, 5vw, 22px)', fontWeight: 700, color: 'var(--text1)', marginBottom: 4 }}>
          Good morning, {user?.displayName || "User"}
        </div>
        <div style={{ fontSize: 'clamp(11px, 3.5vw, 13px)', color: 'var(--text2)' }}>
      ...
      {/* Stats - Using a class for responsive grid */}
      ...
      {/* Main Grid - Using a class for responsive grid */}
      <div className="dashboard-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16 }}>
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {critical.map(p => (
              <div key={p.id} onClick={() => handlePatient(p.id)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 10px', borderRadius: 8, cursor: 'pointer', borderBottom: '1px solid var(--border)', transition: 'background 0.15s' }}
                onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.03)')}
                onMouseLeave={e => (e.currentTarget.style.background = 'none')}>
                <div className="avatar" style={{ width: 36, height: 36, background: p.avatar, fontSize: 12, flexShrink: 0 }}>{p.initials}</div>
        ...
        <div className="card">
          ...
          {ACTIVITY_FEED.map((a, i) => (
            <div key={i} style={{ display: 'flex', gap: 12, padding: '12px 0', borderBottom: i < ACTIVITY_FEED.length - 1 ? '1px solid var(--border)' : 'none' }}>
        ...
        <div className="card">
          ...
          {DOCTORS.map((d, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: i < DOCTORS.length - 1 ? '1px solid var(--border)' : 'none' }}>
